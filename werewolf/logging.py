...
from werewolf.model import Logger, RoundLog, State, to_dict
...
def store(logger:Logger,directory: str = None):
    if directory is None:
        directory = log_directory()
    
    os.makedirs(directory, exist_ok=True)
    with open(f"{directory}/game_logs_{logger.round}.json", "w") as file:
        json.dump({"round": logger.round, "logs": logger.to_json_dict()}, file, indent=4)
