...
class GameView:
  ...
  def set_statement(self, statement: str):
    """Sets the player's statement for the current round."""
    self.statement = statement

  def set_last_thought(self, thought: str):
    """Sets the player's last thought for the current round."""
...
class Player(Deserializable):
  ...
  def _get_game_state(self) -> Dict[str, Any]:
    ...
    return {
        ...
        "last_thought": self.gamestate.last_thought,
        "statement": self.gamestate.statement
    }
...
class Logger(Dict):
  ...
  def add_log(self, log:str, player:str, muteplayers:tuple[str]):
    key = (player, muteplayers)
    self.setdefault(key, {})
    self[key]["log"] = log

  def get_log(self, player, muteplayers):
    return self.get((player, muteplayers), None)

  def add_pvote_log(self, pvote, player:str,muteplayers:tuple[str]):
    key = (player, muteplayers)
    self.setdefault(key, {})
    self[key]["pvote"] = pvote
  def add_nvote_log(self, nvote, player:str,muteplayers:tuple[str]):
    key = (player, muteplayers)
    self.setdefault(key, {})
    self[key]["nvote"] = nvote

  def to_json_dict(self):
    result = {}
    for (player, mute_tuple), data in self.items():
        mute_str = ",".join(mute_tuple) if mute_tuple else ""
        result[f"{player}|{mute_str}"] = data
    return result